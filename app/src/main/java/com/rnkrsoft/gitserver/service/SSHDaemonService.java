...
import com.rnkrsoft.gitserver.ssh.CommandFactory;
import com.rnkrsoft.gitserver.ssh.HostKeyProvider;
import com.rnkrsoft.gitserver.ssh.NoShell;
...
import com.rnkrsoft.gitserver.ui.widget.ToggleAppWidgetProvider;
...
public class SSHDaemonService extends Service implements PasswordAuthenticator, PublickeyAuthenticator {
	...
	public void onDestroy() {
		try {
			if(sshServer != null) {
				...
				Commons.stopStatusBarNotification(this);
			}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		sshServer.setKeyPairProvider(new HostKeyProvider(this));
		sshServer.setShellFactory(new NoShell());
		sshServer.setCommandFactory(new CommandFactory(this));
		...
		try {
			...
			if(isStatusBarNotificationEnabled) {
				Commons.makeStatusBarNotification(this);
			}
	...
	public boolean authenticate(String username, String password, ServerSession session) {
		...
		try {
			...
			String passwordSha1 = Commons.generateSha1(password);
