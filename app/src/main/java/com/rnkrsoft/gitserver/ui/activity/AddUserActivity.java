		...
		if(editMode) {
			...
			try {
				...
				if(password != null && !"".equals(password.trim())) {
					user.setPassword(Commons.generateSha1(password));
				}
			...
			try {
				User checkUser = getHelper().getUserDao().queryForUsername(usernameEditText.getText().toString().trim());
				if(checkUser != null) {
					Toast.makeText(AddUserActivity.this, "用户名已存在", Toast.LENGTH_SHORT).show();
					return;
				}
				
				checkUser = getHelper().getUserDao().queryForEmail(emailEditText.getText().toString().trim());
				if(checkUser != null) {
					Toast.makeText(AddUserActivity.this, "邮箱已存在", Toast.LENGTH_SHORT).show();
					return;
				}
				...
				Log.e(TAG, "SQL problem.", e);
				Toast.makeText(AddUserActivity.this, "数据库错误.", Toast.LENGTH_SHORT).show();
				return;
			}
			
			try {
				getHelper().getUserDao().create(new User(0, fullname, email, username, Commons.generateSha1(password), publickey, active, System.currentTimeMillis()));
			} catch (SQLException e) {
		...
		if(requestCode == CONTACT_PICKER) {
			...
			if(c.moveToFirst()) {
				...
				if(name != null && !"".equals(name.trim())) {
					...
					usernameEditText.setText(Commons.toCamelCase(firstWord));
				}
